describe 'Cipher Block Chaining mode encryption', ->
    secua = nil
    ...
    describe 'correct block size of data', ->
        it 'correctly encrypts with a 128 bit key and 128 multiple block size', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.are.same expected, (encrypt inputdata, inputkey, iv)
        ...
        it 'correctly encrypts with a 256 bit key and 128 multiple block size', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.are.same expected, (encrypt inputdata, inputkey, iv)
        it 'correctly encrypts with a 256 bit key and non 128 multiple block size', ->
            inputkey = { 
                0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
                0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF 
            }
            ...
            expected = {
                0xDF, 0xBF, 0x54, 0xF5, 0xD2, 0x6E, 0x6A, 0x6D, 0x06, 0xC1, 0xC5, 0x17, 0x01, 0x08, 0x35, 0x13,
                0xC0, 0xDF, 0x08, 0x2B, 0xE2, 0x8A, 0x65, 0x74, 0x36, 0x01, 0x66, 0xC7, 0x46, 0xD0, 0x11, 0x93
            }
            encrypt = secua.aes.cbc.encrypt
            assert.are.same expected, (encrypt inputdata, inputkey, iv)
    describe 'Error handling code', ->
        it 'rejects non existent data', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt nil, inputkey, iv)
        ...
        it 'rejects non existent key', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt inputdata, nil, iv)
            assert.stub(print).was.called 1
        it 'rejects non existent IV', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt inputdata, inputkey, nil)
            assert.stub(print).was.called 1
        it 'rejects nil key, input and iv', ->
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt nil, nil, nil)
            assert.stub(print).was.called 1
        it 'rejects non table data', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt 'I cannot think of any good quote to go here', inputkey, iv)
            assert.stub(print).was.called 1
        it 'rejects non table key', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt inputdata, 'Or here', iv)
            assert.stub(print).was.called 1
        it 'rejects non table key, input and iv', ->
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt 'or even here', 'I am so sleep deprived', 'but at least strings are not tables')
            assert.stub(print).was.called 1
        it 'A key of incorrect length', ->
            ...
            encrypt = secua.aes.cbc.encrypt
            assert.is_nil (encrypt inputdata, inputkey, iv)
            assert.stub(print).was.called 1