describe 'State to Byte Utility tests', ->
    secua = nil
    ...
    before_each ->
        stub _G, "print"
    it 'correctly converts a state to bytes', ->
        hexBytify = secua.utils.hexBytify
        expected = hexBytify ""
        input = {
            [0]: {
        ...
        assert.are.same expected, secua.sha3.utils.stateToBytes input
    it 'rejects a non-existent byte array', ->
        result = secua.sha3.utils.stateToBytes!
